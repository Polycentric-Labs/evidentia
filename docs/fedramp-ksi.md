...
| Significant Change Notification schema (validates `SCRForm.to_scn_document()` output) | [`FedRAMP/schemas`](https://github.com/FedRAMP/schemas) (CR26, draft) | vendored under `evidentia_core/fedramp/schemas/`; `evidentia_core.fedramp.validate_scn_document` |
...
| `fedramp-rev5-{low,moderate,high,li-saas}` baseline membership | [`OSCAL-Foundation/fedramp-resources`](https://github.com/OSCAL-Foundation/fedramp-resources) `baselines/rev5/json` (the FedRAMP PMO OSCAL profiles) | `scripts/catalogs/upstream/fedramp-rev5-baselines.json`, produced and checked by `scripts/catalogs/build_fedramp_baselines.py` |

Exact commits, blob SHAs, and sha256 hashes live in
`evidentia_core/fedramp/schemas/UPSTREAM.json`; the baseline profiles'
sha256 and blob SHAs sit in the membership file's `provenance.files`.
All three vendored schemas are byte-identical to upstream (verifiable by `git hash-object` against
...
sentinel run red until the pins are deliberately re-verified. The same
run probes the four Rev 5 baseline profiles at the republisher: a
profile that disappears, or whose extracted membership no longer matches
the vendored file, is MAJOR; metadata-only churn is a NOTICE that asks
for a re-run of the extractor at the next deliberate re-sync.
