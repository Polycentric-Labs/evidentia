...
| 2026-05-18 | v0.9.5 (skip-by-reuse with v0.9.5 feature-table additions) | v0.9.5 (walk-through refinement + collaboration primitives + carry-over closure) is patch+ scope. **Light update**: added 3 new feature rows to the §3.4 capabilities table (POA&M `Milestone.owner` + `Milestone.reviewer` ownership fields with CLI + REST filters; `EvidenceArtifact.version` + `lineage_id` + `new_version()` versioning data-model; `evidentia_core.rbac` package with `Role` / `RBACPolicy` + FastAPI `require_role()` dependency factory). **Also**: added 2 rows to the §6 "Unclaimed gaps" table marking RBAC + evidence-versioning as parity moves (not new differentiators — both are table-stakes for federal-tier deployments; v0.9.5 ships the primitives, v0.9.6 ships the enforcement). **Skip rationale on §1-2 + §4-15**: no material change since v0.7.8 baseline. All 6 skip criteria PASS: (1) doc 16 days old (< 90); (2) patch-equivalent scope; (3) no new enterprise-grade claim (RBAC + versioning are federal-tier table-stakes per §6 prior framing); (4) no competitor category-defining feature; (5) tailwind dates unchanged; (6) threat-model v0.9.5 delta refreshed today. **AI-persona walk-through validation** (P2.1) surfaced FedRAMP 20x / RFC-0024 / OMB M-24-10 framing — incorporated into the federal-SI walk-through, not into the positioning doc itself (the walk-through is the operator-facing artifact; the positioning doc is the strategic-narrative artifact). Next full re-sync target: Q3 2026 (~July 2026), tracked as deferred to v0.9.6 if Q3 has arrived by ship time. |
