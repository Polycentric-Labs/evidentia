...
|---|---|---|
| **Catalog: Control** | `evidentia_core.models.catalog.ControlCatalog` (the control catalogs within the 97-catalog bundle, including NIST 800-53, SOC 2, ISO 27001 stub, CIS, CMMC, PCI DSS and EU AI Act) | Tier-classified (A/B/C/D; see `docs/contributing-a-catalog.md`) with a derived text depth; JSON or YAML format (v0.10.3+ via `docs/contributing-a-catalog.md`). 1:1 conceptual match. |
...
| **Catalog: Guidance** | `docs/*` (operator runbooks, deployment guides, the contributor guide, …) + per-finding `SecurityFinding.remediation` (v0.10.0+) + per-gap `ControlGap.remediation_guidance` | Evidentia's guidance is split between human-readable docs and per-finding remediation text. |
...
| **Log: Evaluation** | `evidentia_eval.DFAHarness` (Determinism-Faithfulness Assurance Harness; v0.8.2+) + faithfulness scoring (Jaccard / sentence-transformers semantic / LLM atomic-claim extraction) | DFAH evaluation results are Gemara-Evaluation-log shaped. |
| **Document: Mapping** | `evidentia_core.models.common.ControlMapping` — OLIR-typed (NIST Open Specification 800-53 to ISO 27001 / CSF / CMMC / etc.) + 15 bundled crosswalks + per-`SecurityFinding.control_mappings[]` provenance | OLIR relationship + justification on every mapping. 1:1 with Gemara Mapping documents. Evidentia's OLIR layer is more relationship-typed than what Gemara currently specifies. |
