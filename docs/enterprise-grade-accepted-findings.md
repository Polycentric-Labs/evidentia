...
| **#181** (v0.12.0) | `packages/evidentia-core/src/evidentia_core/gap_store.py:252` | `path.read_text(encoding="utf-8")` — same sink as #72. | **Re-surfaced #72, not a new finding.** The v0.12.0 `GapReportRepository` seam added ~69 lines above `load_report_by_key`, moving the identical `read_text` sink from line 185 to 252 and re-fingerprinting the dismissal. The code is byte-identical: `_validate_key_shape` still rejects anything that is not exactly `[0-9a-f]{16}` (so `..`, `/` and `\` are unrepresentable), and `validate_within(candidate, store)` still resolves-and-contains before the read. The new caller, `GapReportRepository.load_by_key`, validates the shape a *second* time before delegating. This is the same re-fingerprint class as #170/#171 (which re-surfaced #132/#134). |
| **#182-#192** (v0.13, PR #289) | `packages/evidentia-api/src/evidentia_api/routers/catalog.py:389,401`; `packages/evidentia-collectors/src/evidentia_collectors/sql/sqlite/collector.py:259`; `packages/evidentia-core/src/evidentia_core/evidence_store.py:457,459`; `gap_store.py:235,237`; `metric_store.py:103`; `retention_metadata_store.py:93,95`; `workflow_store.py:92` | The same `exists()`, `is_file()`, `read_text()` and `write_text()` sinks as #132/#133/#170, #174, #137/#138, #71/#72/#181, #141, #144/#145 and #149. | **Re-surfaced, not new findings.** PR #289 ran `ruff format` over the whole repository. The formatter rejoined multi-line calls such as `out_path.write_text(` and `path.read_text(encoding="utf-8")` onto single lines and shifted every line number, so CodeQL's fingerprints for the dismissed alerts no longer matched and the identical sinks reopened under new numbers; the PR check itself noted that the diff was too large to attribute alerts. The code is AST-identical to `main`: the id-shape validation and `validate_within(candidate, base)` guards upstream of every sink are unchanged. |
