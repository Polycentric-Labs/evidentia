...
[collector design](designs/entra-m365-collector-design.md).
...
**Error responses (2026-07-06 error-shape convergence)**: every
deliberate 4xx/5xx raised by a handler carries the structured
...
generalizing the v0.9.5 ``rbac_denied`` 403. The ``error`` keys are a
stable, additions-only vocabulary (registry:
``evidentia_api.errors``); clients MAY dispatch on them. ``message``
texts and site-specific context fields are informative — not frozen.
FastAPI/Pydantic request-validation 422s keep their array-shape
``detail`` (unchanged). Each operation's deliberately-raised 4xx/5xx
statuses are documented in its OpenAPI ``responses``.
...
No `catalog pin` command exists or is scheduled. To hold an
assessment to a fixed catalog version, import the copy you assessed
against with `evidentia catalog import`: a user-imported catalog
shadows the bundled one under the same framework id, and
`evidentia catalog where <framework>` shows which copy resolves.
...
| **NORMATIVE** | **2026-05-26** | **v0.10.6 Phase 1 (commit C1): catalogs. Three new bundled YAML catalogs (`osps-baseline-m1.yaml`, `osps-baseline-m2.yaml`, `osps-baseline-m3.yaml`) bundling the OpenSSF OSPS Baseline at upstream commit `ac6bbec` (Apache-2.0, Tier-A verbatim). Catalog entries are OSPS assessment-requirements (per-maturity-level applicability filter): M1=25, M2=42, M3=63 (counts verified against upstream tarball; the v0.10.6 plan §Phase 1 expected 21/38/58 — upstream actual is used). Companion OSCAL Catalog 1.2.1 serialization shipped at `osps-baseline.oscal.json` (8 groups / 41 top-level OSPS controls, mirroring upstream top-level granularity per OSCAL Catalog convention; validated against Trestle 4.0.3 pydantic models). Additive-only per frozen-surface contract — existing 89 catalogs unchanged. `scripts/catalogs/regenerate_manifest.py` extended with `*.oscal.json` / `*.oscal.yaml` skip-filter so OSCAL sidecar artifacts don't register as manifest entries. Manifest count: 89 → 92 frameworks.** |
...
| **NORMATIVE** | **2026-09-06** | **v0.13 batch 4 (V13-01): the cadence evidence series.** New `evidentia conmon series <slug>` CLI leaf, `POST /api/conmon/series`, and 14th MCP tool `conmon_series` (append-only per §MCP tool contract) all wrap `evidentia_core.conmon.series.assert_series`: a dated series read from the append-only evidence store (artifacts link to a cadence via `metadata.cadence_slug`) instead of the operator-maintained state file `conmon check`/`conmon health` read. `evidentia_mcp.cli.V0_13_NEW_TOOLS` extends the `cimd-migrate` default grant set alongside `V0_9_6_NEW_TOOLS`. No removals, no renames, no breaking changes. See [conmon-runbook.md](conmon-runbook.md) § Cadence evidence series and [designs/cadence-assertion-layer-design.md](designs/cadence-assertion-layer-design.md).** |
...
| **NORMATIVE** | **2026-09-09** | **v0.13 catalog currency (V13-09, first batch).** Additive optional `CatalogControl.properties`, `ControlCatalog.status` / `notes` / `verified_on` / `superseded_by` / `audit_contexts` / `publication_notices`, `CatalogStatus`, `CatalogAuditContext` and `CatalogPublicationNotice`. Manifest and registry discovery carry lifecycle metadata, and CLI/API imports preserve validated currency and licensing fields. Native loading also preserves the already declared family hierarchy, EU risk categories and legacy note. Publication notices never enter control indexes, text-depth calculations or assessment denominators. Existing metadata defaults remain unknown or empty; dates do not activate requirements. Existing endpoints and CLI leaves expose these fields without removals or renames. API YAML dates serialize as ISO strings; unsupported non-JSON values return the documented 400 invalid_body before replacement. Imported license/source strings render literally in the CLI. Catalog IDs and statement content remain the non-frozen data surface; the CMMC and CISA content migrations imply no automatic equivalence. |
...
[SCAP operator guide](scap-collectors.md) for exact limits and error behavior.
...
[catalog guide](wiki/2-guides/manage-catalogs.md#publication-and-storage-limits).
