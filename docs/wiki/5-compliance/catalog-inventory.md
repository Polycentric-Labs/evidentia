...
The current distribution across all 107 catalogs, derived from the manifest and the
...
## Pinned native source coverage

- **Australian ISM 2026.09.4:** 1,143 indexed controls and 49 principles retained as native context. The 24 former heading IDs are retired without automatic replacements. Select current controls explicitly; existing evidence is not rewritten. Publisher attribution and exceptions remain with the retained source README.
- **CISA SCuBA for Microsoft 365:** eight pinned Markdown baselines provide 128 policy identities. The accompanying reference JSON has 127 records and remains separate. This catalog does not execute ScubaGear checks or cover Google Workspace. Repository CC0, Microsoft-adapted CC BY 4.0 notices and product-license prerequisites remain attached to their source documents.
- **BSI Grundschutz++:** an external-only importer accepts the exact three operator-selected files for profile `bsi-grundschutz-plus-plus-367d7750`; source text is not bundled. Import does not grant or accept rights for the operator.
- **FINOS CCC:** adoption remains deferred for owner legal review. FINOS sources and normalized catalogs remain excluded from this release; future adoption requires an explicit reviewed scope.

Native source occurrence counts describe retained structure, not additional assessment controls. See [Manage catalogs](../2-guides/manage-catalogs.md#native-source-catalogs) for source browsing, import and storage limits.
