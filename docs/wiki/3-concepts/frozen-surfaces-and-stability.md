...
- **Pydantic model fields** (`evidentia_core.models.*`). All exported model classes have stable field names and types. Adding an optional field (with a default) is a minor change; renaming, removing, or retyping a field is a major trigger. The frozen set is every module of `evidentia_core.models` plus the AI-governance, RBAC and retention model modules the api-stability table lists: `SecurityFinding` / `Finding` / `ComplianceStatus` / `FindingStatus`, `ControlGap` / `Milestone` / `POAMState`, `CrosswalkDefinition` / `FrameworkMapping`, `CatalogControl` / `ControlCatalog`, and the rest (see the [data model](data-model.md) page and the api-stability table for the full roster). `scripts/check_public_surface.py` verifies every row of that table: the module file exists, every listed class and callable is defined, and every `evidentia_core/models/*.py` module has a row. A serialization guarantee rides along: JSON output of a frozen model at version N must deserialize under version N+1 within the same major (field *ordering* is not guaranteed).
