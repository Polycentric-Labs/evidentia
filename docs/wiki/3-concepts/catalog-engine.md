...
A *catalog* is the machine-readable representation of a compliance framework's controls: NIST SP 800-53 Rev 5, SOC 2 TSC, ISO 27001, the OpenSSF OSPS Baseline, and some 90 others. Catalogs are the "target state" in gap analysis: the gap analyzer compares your evidence against a catalog's controls and reports what's missing. This page explains how catalogs are stored, discovered, loaded, validated, and indexed, and how to regenerate the manifest after adding one.

The code lives in `packages/evidentia-core/src/evidentia_core/catalogs/`.

## The manifest is the single source of truth

Bundled catalogs are enumerated in one file: `catalogs/data/frameworks.yaml`. As of v0.13 it lists **97 frameworks**. The manifest's header makes the contract explicit:
...
- `FrameworkManifestEntry`, one framework. Fields: `id` (canonical kebab-case ID, stable across versions), `name`, `version`, `tier`, `category`, `path` (catalog file path relative to `data/`), `source_url`, `license`, `license_required`, `license_url`, `placeholder`, `extras` (install-extra gating, e.g. `"stigs"`), `refresh` (the CI refresh cadence), `text_depth` (derived, see below) and `crosswalk_family` (the parent catalog whose crosswalks also apply; see the [crosswalk engine](crosswalk-engine.md)). The record uses `extra="forbid"`, so a malformed manifest entry fails loudly at load.
...
The catalog models and manifest carry metadata that the rest of the engine reads:

- **`Tier`** = `"A" | "B" | "C" | "D"` is the *redistribution posture*, nothing more. A = public-domain or open-licensed, redistributable verbatim (US federal works, Apache-2.0, CC-BY); B = free to use with attribution (MITRE ATT&CK, CISA KEV); C = copyrighted, so only the public numbering and neutral titles ship as a placeholder and the operator supplies their licensed copy (ISO, SOC 2 TSC, PCI DSS, HITRUST, CIS); D = government regulation or statute, restated as obligations (GDPR, NIS2, DORA, state privacy laws). The per-tier rules are in the [contributor guide](../5-compliance/contributing-a-catalog.md); each manifest entry's `license` and `source_url` carry the per-catalog statement (`evidentia catalog license-info <id>` prints them).
- **`TextDepth`** = `"full" | "partial" | "headings"` says how much control text a catalog actually carries, and it is derived from the catalog file rather than declared: `full` when every non-withdrawn entry has statement text that differs from its title, `headings` when none does (the catalog carries control numbering and titles only), `partial` otherwise. `derive_text_depth` in `models/catalog.py` is the rule, every catalog model exposes it as `.text_depth`, and `regenerate_manifest.py` writes the result into the manifest's `text_depth` column. Tier and depth are independent: 26 Tier A catalogs are heading-only, and the truth gate (`scripts/check_catalog_truth.py`) fails if the column ever disagrees with the files.
...
Each `CatalogControl` carries `id`, `title`, `description`, `family`, OSCAL-derived fields (`objective`, `guidance`, `priority`, `baseline_impact`, `related_controls`, `assessment_objectives`, `parameters`), nested `enhancements: list[CatalogControl]`, per-control tier/license metadata, and `withdrawn` (set from the OSCAL `status` prop; NIST SP 800-53 Rev 5 keeps 182 withdrawn entries so references keep resolving, and the gap analyzer skips them). AI-governance catalogs additionally use `risk_tier` and `applies_to_annex_iii` (v0.9.3).
...
2. Run `uv run python scripts/catalogs/regenerate_manifest.py`. The script scans each tier directory, accepts `.json` / `.yaml` / `.yml` catalog files, infers a best-guess refresh cadence from tier + category, loads every catalog to derive its `text_depth`, writes `crosswalk_family` from its family table, and rewrites `frameworks.yaml` so the manifest is *truthful by construction*: the contents of `data/<tier>/` **are** the manifest, no hand-maintained sync.
