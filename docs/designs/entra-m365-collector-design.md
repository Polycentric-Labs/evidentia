...
This design defines bounded identity, device, retention, DLP configuration, and Defender event evidence. Typed contracts, a Graph reader, admission accounting, authored mappings, and fixture provenance form the shared foundation. Domain readers and the collector, CLI, API, and console entry points implement this contract. Their tests exercise synthetic Graph transport and recorded DLP configuration; they do not establish live-tenant acceptance.
...
The request has exactly these fields. Duplicate/unknown keys and invalid types fail before credential lookup or collection. Numeric booleans, strings, coercion, and format guessing are rejected.
