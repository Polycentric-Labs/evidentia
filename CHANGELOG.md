...
- **Collectors SSRF guard (threat-model T2).** The OCSF collector's private-IP refusal (the v0.10.2 `--block-private-ips`) is lifted into a shared `evidentia_core.network_guard.enforce_public_host` and applied **default-on** to all 13 outbound collectors (okta, databricks, snowflake, github, vanta, drata, bitsight, securityscorecard + the 4 SQL adapters) and the API collectors router — closing the surface where an operator-supplied host (or, on an exposed `serve`, an anonymous request) could reach cloud-metadata / internal endpoints. Secure-by-default: internal-endpoint collection now requires the opt-out (`--allow-private-ips` / `block_private_ips=false`); fail-closed on unresolvable hosts. 54 new SSRF tests. (AWS is unaffected — boto3 resolves SDK endpoints, not an operator host.)
