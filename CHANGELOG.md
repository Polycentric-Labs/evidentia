...
- **Upstream release publication cadence (V13-28).** `collect release-cadence`,
  ...
  remediation or compliance. See the [release-cadence guide](docs/wiki/2-guides/release-cadence.md).
...
- **Explicit CycloneDX 1.7 exports (V13-26).** Gap-report VEX exports retain
  ...
  [CycloneDX export guide](docs/wiki/2-guides/emit-cyclonedx-vex.md).
...
- **Native catalog sources (V13-08).** Australian ISM and CISA Microsoft 365
  ...
  pending owner legal review. See [Manage catalogs](docs/wiki/2-guides/manage-catalogs.md).
...
- **Local SCAP imports (V13-25).** `collect scap`, its raw XML API and the
  ...
  See the [SCAP operator guide](docs/scap-collectors.md).
...
- **Incident workflow clocks (V13-06).** `collect incident-clock`, its API
  ...
  [incident clock design](docs/designs/incident-clock-collector-design.md).
...
- **Enterprise retention configuration (V13-04).** `collect enterprise-retention`,
  ...
  Trusted profiles separate exact API-principal grants from local CLI permission.
  Full results retain native values, every selected resource and the unique read
  ledger, including partial and unavailable evidence. CLI output is atomic; API
  output and console downloads preserve the validated complete JSON. Authored
  synthetic fixtures and demo scenarios do not establish live-provider acceptance.
  Vault retention rules, effective enforcement and record coverage remain unassessed.
  See [the enterprise design](docs/designs/enterprise-retention-collector-design.md).
- **Storage retention configuration (V13-04, storage milestone).** A new
  ...
  object enforcement, complete recordsets or compliance. Enterprise retention
  supplies the separate Vault, Splunk and Elastic surface; Entra/M365 supplies
  retention-label configuration evidence.
...
- **Catalog currency notices (V13-09, first catalog batch).** Optional lifecycle,
  ...
  explicit notices. See [Catalog currency](docs/catalog-currency.md).
...
- **Quality corrections.** Catalog listing and diagnostics distinguish missing
  ...
  transaction for its temporary-table privilege probe and stops when the probe
  savepoint cannot be restored. Secondary transaction-cleanup interruptions
  preserve the directly captured probe failure. Fuzz harness cleanup preserves
...
**Theme**: *Day-N dependency sweep on a fresh hardened base.* A security patch that clears **all 26 open Dependabot advisories** across the three manifests (`uv.lock` 11, `packages/evidentia-ui/package-lock.json` 11, `docker/requirements.txt` 4) in one consolidated bump, rebuilds the published image on the current DHI base (the drift the base-freshness sentinel flagged in issue #218), and fixes a shipped packaging defect in the `evidentia-ai[eval-faithfulness]` extra. Per `SECURITY.md` §Supported versions the latest patch is the only release guaranteed free of disclosed advisories — v0.11.0 no longer was. The required `osv-scanner (SBOM)` check had been failing on every open PR because the Python SBOM carried aiohttp + cryptography + pyasn1 advisories that no single-package Dependabot PR could clear together; this release lands all three at once, so the Dependabot queue (parked at its 15-PR cap) drains again. This tag also ships everything that merged to `main` after v0.11.0 (the README brand refresh, below). No package (`packages/*/src`) code changes beyond the version bump. (The **0.11.1 tag was created but never published** — the atomic release's validate-before-publish design stopped it: every pre-publish gate and the full artifact build passed, then the publish job's `twine check` rejected the wheels because hatchling 1.32.0 (2026-08-11) had bumped their core-metadata version to 2.5 and the pinned `pypa/gh-action-pypi-publish` v1.14.0 still bundled twine 6.1.0; nothing reached PyPI, GHCR, or Releases. Per the ghost-tag runbook the tag stays — protected, append-only — and 0.11.2 re-releases the identical tree with the action bumped to v1.14.2 / twine 7.0.0, below.)

### Security

- **Python (`uv.lock`)** — `cryptography` 46.0.7 → **50.0.0** (CVE-2026-69247 PKCS#7 Bleichenbacher oracle, CVE-2026-69248 wildcard-DNS `permittedSubtrees` escape, CVE-2026-69249 exponential path building; fixed 50.0.0 / 49.0.0); `aiohttp` 3.14.1 → **3.14.3** (CVE-2026-69244 OOB heap read, CVE-2026-69243 request smuggling via WebSocket upgrade, CVE-2026-59881 unnegotiated compressed frames); `pyasn1` 0.6.3 → **0.6.4** (CVE-2026-59884/-59885/-59886 long-form tag-ID DoS, quadratic OID, REAL-value resource consumption); `pymdown-extensions` 10.21.3 → **11.0.1** (CVE-2026-67422 ReDoS + b64 path traversal, CVE-2026-61632). `pyopenssl` floats 26.0.0 → 26.4.0 alongside cryptography.
- **Container (`docker/requirements.txt`)** — regenerated against the freshly built 0.11.1 wheels: `cryptography` 49.0.0 → 50.0.0 (CVE-2026-69247), `aiohttp` 3.14.1 → 3.14.3 (CVE-2026-69244 / -69243 / -59881). Both pinned bases move to their current digests: builder `python:3.13-slim` (`eb43ff12…` → `ffb752e1…`, pip 26.2.1) and final `dhi.io/python:3.13` (`1842a6b9…` → `08150637…`, the digest the sentinel has reported current since 2026-07-28); the image is rebuilt, smoke-tested, re-signed and re-attested end-to-end by `release.yml`. Closes issue #218; supersedes Dependabot PR #203 (whose 07-20 DHI digest no longer resolves).
- **Web UI (`packages/evidentia-ui/package-lock.json`)** — `react-router` / `react-router-dom` 7.18.1 → **7.18.2** (GHSA-qwww-vcr4-c8h2 RSC-mode CSRF bypass; the only runtime-scope npm advisory); dev-scope: `js-yaml` 4.2.0 → 4.3.1 (GHSA-5p4m-2wfm-xmqj, CVE-2026-59869 quadratic CPU; the `overrides` pin follows), `postcss` 8.5.19 → 8.5.26 (CVE-2026-69153 arbitrary `.map` read via `sourceMappingURL`), `undici` 7.28.0 → 7.29.0 (CVE-2026-13697 / -15157 / -14643 / -16729 / -16728), `brace-expansion` 2.1.1 → 2.1.4 and 5.0.6 → 5.0.9 (CVE-2026-13149 exponential `{}` expansion). `npm audit`: 0.
- **`py-ocsf-models` range widened 0.9.x → `>=0.9.0,<0.11.0`** (evidentia-core `[ocsf]`, evidentia-collectors `[ocsf]`/`[all]`, root dev-deps): 0.10.0 (2026-07-13) dropped the `cryptography>=44.0.3,<47` cap that had pinned the full-extras dev lock at 46.0.7 and blocked the cryptography fixes above; the OCSF mapping, round-trip, collector and CLI suites pass unchanged on 0.10.0.
- **`osv-scanner.toml`: zero active allowlist entries.** The accepted cryptography GHSA-537c-gmf6-5ccf entry (dev-SBOM-only, accepted 2026-06-15) is REMOVED — its documented removal trigger fired (the py-ocsf-models cap lift above). The history note stays in the file per the allowlist convention.

### Fixed

- **`evidentia-ai[eval-faithfulness]` was unsatisfiable on a clean install** — the extra pinned `evidentia-eval[faithfulness-semantic]>=0.10.4,<0.11.0` against a base dependency of `evidentia-eval>=0.11.0,<0.12.0` (masked locally by uv workspace path resolution; a PyPI install of the extra hit a resolver conflict). Root cause: `scripts/bump_version.py`'s inter-package pin regex required the bare member name to be followed directly by `>=`, so a PEP 508 extras bracket was silently skipped at every sweep. The regex now admits `[extras]` inside the preserved name group (regression test added), and the pin is `>=0.11.1,<0.12.0`.
- **Release publish step accepts core-metadata 2.5** — `pypa/gh-action-pypi-publish` v1.14.0 → **v1.14.2** (`release.yml`; twine 6.1.0 → 7.0.0, "Fix uploading packages with metadata version 2.5", twine #1317). hatchling 1.32.0 emits `Metadata-Version: 2.5` by default with no option to pin it back, and PyPI already accepts 2.5 (hatchling's own wheel is 2.5) — only the action's bundled twine was behind. This is the defect that turned v0.11.1 into a ghost tag (theme paragraph above); it is the release workflow's first live execution on the new pin, per the first-live-run audit.
- **gitleaks allowlist** (`.gitleaks.toml`) — the two v0.12 gap-report-repository seam test modules (`tests/integration/test_api/test_gap_report_repository_router.py`, `tests/unit/test_gap_analyzer/test_gap_store_repository.py`) join the existing module-scoped allowlist for fake 16-hex `report_key` fixtures (the same generic-api-key false-positive class as `test_gaps.py`). Recorded because the CI scan fetches every branch and runs history-wide, so a false positive on any pushed branch fails the required `gitleaks` check for every open PR — which is exactly what happened when the seam branch was pushed.
- **Release regeneration tooling** — the pip-compile regen (`release.yml`, `container-build.yml`, `bump_version.py --regenerate-requirements`) moves `pip-tools` 7.5.3 → **7.6.1**: 7.5.3 breaks on the pip 26.2 shipped in the current `python:3.13-slim` (`make_requirement_preparer() missing 'allow_editables'`), which would have failed the tag-time regen. `bump_version.py` now reads the builder digest from the `Dockerfile` (single source of truth; the script's own literal had drifted behind it — a unit test asserts the two workflows carry the same pin), preserves non-`evidentia` lines in `docker/requirements.in` (the deliberate `urllib3>=2.7.0` floor was being dropped), and no longer requires Git Bash's `pwd -W` on Windows.
...
**Deferred (ratified v0.12 plan re-cut, 2026-07-18 — see [`docs/releases/plans/v0.12-plan.md`](docs/releases/plans/v0.12-plan.md))**: Wave 4 phase 3 (EU-AI-Act ↔ ISO 42001 crosswalk) → v0.12-if-capacity; Wave 4 phases 1/2/4, Wave 3 DORA, and the medical-device traceability enrichment → v1.1; Waves 5-6 (OpenVEX/VSA, SARIF collector, auto-review) → post-1.0; arXiv preprint timing = open decision. The GUI parity pass for the five federal api-only verbs leads v0.12.
...
- **PEP 770 per-wheel SBOMs** — every published wheel now embeds a
  deterministic CycloneDX SBOM in `.dist-info/sboms/` (generated by
  `scripts/gen_package_sboms.py`, injected by a per-package hatchling build
  hook, skip-clean for local builds without it). The `evidentia-api` wheel
  additionally embeds the bundled React SPA's npm-closure SBOM — the
  phantom-dependency surface Python tooling can't see. The release build
  generates once before its reproducible double-build and fails if any
  wheel is missing its SBOM.
...
- **REST error payloads converge on one machine-readable shape** — every
  ...
  `{detail, next_version}` 409 (its `next_version` recovery hint keeps its
...
- **Config-test isolation** — `find_config_file` gained an optional `stop_at` boundary; the config unit + API tests now bound the upward `evidentia.yaml` walk to their tmp tree, so a stray ancestor config (e.g. a `~/evidentia.yaml` — pytest tmp dirs live under `$HOME`) can no longer leak into the suite and cause spurious failures.
...
**Theme**: *v0.10.7 — web console (GUI v2) refresh + gap-report export + hygiene / automation-debt / wiki-fill / doc-accuracy base*. Patch bump (v0.10.6 → v0.10.7). Last cycle on the v0.10.x line before the v0.11 federal-compliance theme.

**Release summary**: A web-UI + hardening + automation-debt + documentation cycle. The headline end-user change is the **web console**: a full GUI v2 visual refresh (federal-blue/deep-navy design system, light/dark, self-hosted IBM Plex + brand assets, every route restyled) plus a real **gap-report export/download** surface (all 8 formats) guarded by an OpenAPI type-parity drift-gate. On the hardening side it closes the v0.10.6 code-quality reviewer backlog (Groups A + D), adds a blocking pre-push gate (Layer 2) — now also carrying a never-skip version-anchor guard and a frontend version-literal guard — fills the in-repo wiki tree (~47 pages) with generators wired into CI, adds 7 operator-walkthrough guides, fixes two real product bugs (enum-field rendering in the TPRM + governance CLIs), and runs a doc-wide CLI-example accuracy sweep that surfaced two multi-cycle naming/usage drifts (see §12 corrections). **3863 tests pass / 14 skipped; mypy strict 0/0 across 272 source files; ruff clean.** (The Phase-E version-tracker + README + commit-subject tooling added the tests beyond the mid-cycle 3666 baseline.) Workspace ships **8 PyPI packages** unchanged from v0.10.6 (no new packages this cycle, no LL-V105-1 partial-publish recurrence risk). Per-cycle detail in [`docs/v0.10.7-plan.md`](docs/releases/plans/v0.10.7-plan.md).

### Added

- **Web console — GUI v2 visual refresh.** A full design-system pass: federal-blue interactive primary on a warm off-white workspace with deep-navy brand chrome (nav rail + top bar), the CLI-matched severity palette preserved verbatim, self-hosted IBM Plex Sans/Mono + favicons / PWA manifest / Open-Graph brand assets (air-gap clean, no CDN), a wired light/dark toggle (with a no-flash inline theme script), and every route + the onboarding flow restyled. Presentation only — all API / SSE / Zustand wiring, the OpenAPI type contract, severity hues, and accessibility are preserved; live-validated across all 8 routes with zero console errors. See commits `499be2e`, `fa1c56d`, `37a7b62`, `aa2e0bd`.
- **Web console — gap-report export/download.** The Gap Analyze results expose a one-click export to all 8 report formats (`json`, `oscal-ar`, `sarif`, `ocsf`, `ocsf-detection`, `cyclonedx-vex`, `csv`, `markdown`) wired to the real `POST /api/gap/export` blob download, plus an OpenAPI → TypeScript type-generation step and a CI **type-parity drift-gate** so the typed frontend client cannot silently diverge from the API schema. See commits `4aea58f`, `76192d4`.
...
- **7 operator-walkthrough guides + an input-schema reference** in the in-repo wiki (`docs/wiki/2-guides/` + `docs/wiki/4-reference/`): generate-and-quantify-risk, manage-third-party-risk, manage-model-risk, governance-metrics-and-workflows, ai-governance, mcp-client-setup, serve-the-web-ui, plus an inventory / system-context schema reference. See commit `c52c801`.
...
- **TPRM + governance CLI rendered enum-valued fields raw** (a real product bug). `evidentia tprm dd-questionnaire ingest` crashed, and the governance workflow `run` / `advance` status output showed the wrong value, because the Pydantic models store enum fields as plain strings under `use_enum_values` so `.value` was being called on a `str`. Fixed via the shared `evidentia_core.models.common.enum_value` helper, TDD, with CLI regression tests and a sibling-call-site audit. See commit `777d2a7`.
- **`poam milestone` commands now accept a unique milestone-id prefix** (matching the other id-prefix lookups) instead of requiring the full id. See commit `fd0950b`.
...
### Repo hygiene + version-tracking hardening (Phase E)

- **Never-skip version-tracking.** New declarative manifest `scripts/version_tracked_files.yaml` (`tracked` + `frozen` lists) is the single source of truth that both the bumper and the new gate read. `scripts/bump_version.py` is refactored to drive its replacements from the manifest — and now also tracks `CITATION.cff` (`version` + `date-released`) and the `README.md` container-image tag (tightly anchored), which it previously skipped silently. New `scripts/check_version_consistency.py` hard-fails (pre-tag, wired as the 8th `.githooks/pre-push` check) on any project-version literal in a file that is neither `tracked` nor `frozen`, so a newly-added version reference can no longer drift unnoticed. See commits `eedde81`, `5f44982`, `458a94c`.
- **Live-version-in-frozen anchors + frontend version-literal guard.** A `current-version-anchor` manifest class pins the single live-version literal inside otherwise-frozen files (e.g. the air-gapped-docker block), guarded to require exactly one live literal per anchored line; a `version-decisions` manifest + a decisions-documented gate require every manifest entry to carry a rationale; and `check_frontend_no_hardcoded_version` (the 9th pre-push check) hard-fails on a project-version literal anywhere in `packages/evidentia-ui/src/**/*.tsx`, closing the blind spot left by freezing the evidentia-ui tree wholesale (the UI renders its version from the API). See commits `506ad52`, `9dae2c8`, `691de51`, `4dc1282`, `d2fc25b`, `9088f27`.
- **`openapi.json` tracked + `static/` ignore widened.** The committed OpenAPI schema the frontend type-gen reads is classified as a tracked version-anchor so it stays in lockstep with the release, and the `evidentia_api/static/` ignore rule is widened to a catch-all (`static/*` + `!.gitkeep`) so the web-UI build output — including the new public/ brand assets — cannot leak into git. See commits `34b6288`, `0fe9f26`.
- **CITATION.cff was stale** at `version: 0.7.8` / `date-released: 2026-05-02` (~14 releases behind, because nothing tracked it) and its abstract said "82 framework catalogs" — corrected to `0.10.7` / `2026-05-29` / "92", and now auto-tracked so it can't drift again. See commits `5f44982`, `d33147a`.
- **README "Recent Releases" auto-generated** from the top-3 CHANGELOG version blocks via new `scripts/gen_readme_releases.py` (+ `--check`), rotated to v0.10.7 / v0.10.6 / v0.10.5 (was stale at v0.10.6/5/4). README section headers Title-Cased (`What's in the Box`, `Recent Releases`, `Community & Governance`, `AI Assistance`). Two new `check_docs_health.py` pre-tag invariants — `readme_header_titlecase` + `readme_recent_releases_current` — keep both consistent every release. See commits `434d496`, `8c3ec57`.
- **Capitalized-subject commit convention** enforced by the `.githooks/commit-msg` gate (conventional descriptions after `type:` start uppercase; code-identifier-leading descriptions exempt). See commit `c58012d`.
- **GitHub About managed as code** — `.github/repo-description.txt` is the tracked source of truth, re-asserted to the repo description at release via a documented (Tier-4) `gh repo edit` checklist step, so it can't silently drift. See commit `6f90e3b`.
- **LICENSE** copyright line simplified (dropped the "(formerly ControlBridge)" parenthetical). See commit `bbecf0e`.
- **`docs/archive/` introduced**; `RENAMED.md` moved there (history preserved) with inbound references repointed and the ControlBridge-audit exclusion updated. See commit `21dafab`.
- **Pre-tag review refinements.** (1) Marketplace plugin: `.claude-plugin/plugin.json` is `tracked` so it mirrors the `evidentia-mcp` release per the plugin's own "tracks the release line" design; the plugin README is `frozen` with unpinned install examples + version-agnostic versioning prose (and its stale "12 MCP tools" count corrected to 13), so neither can drift. (2) The README "What's in the Box" table drops the exact test/source-file counts and the per-release PROCEED-CLEAN streak — volatile, non-version stats the version gate can't guard — in favor of a durable "3,700+ tests; mypy strict; ruff clean" floor; live test/coverage health is already shown by the CI + Codecov badges.

### §12 corrections-log entries this cycle

Two accuracy corrections were caught by the documentation verify-everything pass and fixed in the canonical surfaces this cycle (full detail in [`docs/v0.10.7-plan.md`](docs/releases/plans/v0.10.7-plan.md) §12):
...
without scope loss — see [`docs/v0.10.5-plan.md`](docs/releases/plans/v0.10.5-plan.md)
...
- **All 11 remaining collectors populate `compliance_status`
  ...
  [`docs/v0.10.1-plan.md`](docs/releases/plans/v0.10.1-plan.md) §4 (gaps → FAIL,
...
- v0.10.1 pre-release-review opens one new finding: **F-V101-L1
  ...
  [`docs/v0.10.2-plan.md`](docs/releases/plans/v0.10.2-plan.md) Phase 3.
...
[`docs/v0.8.0-plan.md`](docs/releases/plans/v0.8.0-plan.md) (the OSS-native AI
...
- **`.github/actions/gap-analysis/action.yml`** — all four
  ...
  [`docs/v0.7.3-plan.md`](docs/releases/plans/v0.7.3-plan.md).
...
[`docs/v0.7.3-plan.md`](docs/releases/plans/v0.7.3-plan.md) for the next release
...
[`docs/v0.7.1-plan.md`](docs/releases/plans/v0.7.1-plan.md) **moved to**
[`docs/v0.7.2-plan.md`](docs/releases/plans/v0.7.2-plan.md) so v0.7.1 could land
...
to v0.7.2. See [`docs/v0.7.1-plan.md`](docs/releases/plans/v0.7.1-plan.md) (now
SHIPPED) for the line-item ship summary and
[`docs/v0.7.2-plan.md`](docs/releases/plans/v0.7.2-plan.md) for the forward plan.
