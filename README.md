...
- **17 evidence collectors**: AWS, GitHub (including OSPS conformance helpers), Postgres, MySQL, Oracle, SQLite, MS-SQL, Snowflake, Databricks, Okta, Google Workspace, Entra ID / Microsoft 365, storage retention (S3, Azure Blob and GCS), Vanta, Drata, BitSight and SecurityScorecard. Findings use OCSF-aligned compliance status. Entra/M365 and storage retention also return explicit source coverage; storage configuration observations leave compliance unknown.
