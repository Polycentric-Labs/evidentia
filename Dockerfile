...
#
# v0.8.2 G4 (deferred to v0.8.3): the structural foundation
# landed (`docker/requirements.txt` regenerated against the
# v0.8.2 dep tree; `bump_version.py --regenerate-requirements`
# wired). Activation of `pip install --require-hashes -r
# /tmp/requirements.txt` deferred per §25.6 R1: release.yml
# `uv build` is not byte-identical across build hosts (no
# SOURCE_DATE_EPOCH yet), so the SHA256 hashes computed by a
# pre-tag local pip-compile don't match what release.yml
# uploads to PyPI. v0.8.3 will close this via either
# SOURCE_DATE_EPOCH-driven reproducible builds OR a release.yml
# step that regenerates requirements.txt against PyPI after
# Wait-for-PyPI-propagation. For now the install line uses
# exact-version pinning + the wheel's PEP 740 attestation +
# Sigstore signature carry the integrity story.
#
# Recurring Scorecard PinnedDependencies false-positive policy:
# the alert pattern still re-fires on each version bump. See
# docs/dockerfile-pinning.md "Historical narrative" section for
# the per-release dismissal runbook (used at v0.7.12 → v0.8.1
# scans).
