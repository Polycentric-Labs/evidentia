...
#
# v0.8.3 G4 ACTIVATED: hash-pinned install via
# `pip install --require-hashes -r /tmp/requirements.txt`. The
# `docker/requirements.txt` file is regenerated by
# `scripts/bump_version.py --regenerate-requirements --to X.Y.Z`
# which wraps `uv build` (with SOURCE_DATE_EPOCH from HEAD's
# commit timestamp) + `pip-compile --find-links=./dist/`
# producing SHA256 hashes that are byte-identical to what
# release.yml uploads to PyPI (because release.yml uses the
# same SOURCE_DATE_EPOCH-driven reproducible build).
#
# This closes the recurring Scorecard PinnedDependencies false-
# positive cycle (alerts #100 → #115 across v0.7.12 → v0.8.2)
# structurally + permanently. The install line no longer
# matches the alert pattern's regex; future releases get
# Scorecard PinnedDependencies score 10/10 without per-cycle
# dismissals.
#
# Each image build is reproducible byte-for-byte for the same
# (Dockerfile, base-image-digest, requirements.txt) triple. The
# wheels themselves continue to ship PEP 740 attestations +
# Sigstore signatures (validated via `pypi-attestations verify
# pypi`). See docs/dockerfile-pinning.md for the v0.8.3 G4
# closure narrative.
