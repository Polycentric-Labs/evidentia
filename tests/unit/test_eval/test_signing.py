...
class TestSignEvalResult:
    def test_json_written_to_output_path(self, tmp_path: Path) -> None:
        ...
        with mock.patch("evidentia_eval.signing.sign_file") as mock_sign:
            mock_sign.return_value = output.with_suffix(".json.sigstore.json")
    ...
    def test_default_bundle_path_appended(self, tmp_path: Path) -> None:
    ...
    def test_explicit_bundle_path_honored(self, tmp_path: Path) -> None:
        ...
        with mock.patch("evidentia_eval.signing.sign_file") as mock_sign:
            ...
            _out, bundle_path = sign_eval_result(result, output, bundle_path=explicit_bundle)

        assert bundle_path == explicit_bundle

    def test_missing_parent_dir_raises_filenotfound(self, tmp_path: Path) -> None:
    ...
    def test_audit_event_fires(self, tmp_path: Path) -> None:
        ...
        with mock.patch("evidentia_eval.signing.sign_file") as mock_sign:
            mock_sign.return_value = output.with_suffix(".json.sigstore.json")
            with mock.patch("evidentia_eval.signing._log") as mock_log:
                sign_eval_result(result, output)

        # _log.info was called at least once with action=AI_EVAL_OUTPUT_SIGNED.
        action_args = [call.kwargs.get("action") for call in mock_log.info.call_args_list]
        # Compare via the enum value (str enum) for robustness across
        # mypy-strict + runtime contexts.
        assert any(getattr(a, "value", a) == "evidentia.ai.eval_output_signed" for a in action_args)
...
class TestVerifyEvalResult:
    def test_verify_delegates_to_canonical(self, tmp_path: Path) -> None:
        ...
        with mock.patch("evidentia_eval.signing.verify_file") as mock_verify:
    ...
    def test_verify_passes_through_kwargs(self, tmp_path: Path) -> None:
...
class TestVerifyCli:
    ...
    def _isolate_from_ci_rendering(self, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def test_identity_without_issuer_is_usage_error_exit_2(self, tmp_path: Path) -> None:
    ...
    def test_issuer_without_identity_is_usage_error_exit_2(self, tmp_path: Path) -> None:
    ...
    def test_flagless_verify_warns_identity_not_verified(self, tmp_path: Path) -> None:
        """F-V109-2: flagless verify stays exit 0 (back-compat) but
        warns that ANY Sigstore identity would have passed."""
        output = self._write_output(tmp_path)
        runner = CliRunner()
        with mock.patch("evidentia_eval.signing.verify_eval_result") as mock_verify:
