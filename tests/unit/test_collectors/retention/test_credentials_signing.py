...
@pytest.mark.parametrize(
    "version",
    ["1.43.92", "1.43.93", "1.43.96", "1.44.0", "1.43.95+local", None],
    ids=["previously-reviewed", "older", "newer", "minor", "local", "invalid"],
)
...
def test_module_and_distribution_versions_must_agree(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(signing, "_package_version", lambda name: "1.43.95")
...
def test_missing_import_identity_is_preserved(name: str, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(signing, "_package_version", lambda package_name: "1.43.95")
