...
class TestGetVendorStoreDir:
    ...
    def test_default_uses_platformdirs_when_no_env_no_override(self, monkeypatch: pytest.MonkeyPatch) -> None:
...
class TestSaveAndLoad:
    ...
    def test_load_by_id_returns_equivalent_record(self, tmp_path: Path) -> None:
        ...
        original.evidence_refs = [EvidenceRef(title="SOC 2 Type II", artifact_id="abc-123")]
        ...
        assert RegulatoryClassification.CRITICAL_THIRD_PARTY.value in loaded.regulatory_classification
    ...
    def test_save_atomic_no_tmp_file_remains_after_success(self, tmp_path: Path) -> None:
...
class TestIdShapeValidation:
    ...
    def test_save_rejects_id_shape_violation(self, tmp_path: Path) -> None:
...
class TestListVendors:
    ...
    def test_sorted_by_criticality_then_name(self, tmp_path: Path) -> None:
        ...
        v_critical = _make_vendor(name="MMM Critical", tier=CriticalityTier.CRITICAL)
...
class TestDeleteVendor:
    def test_delete_returns_true_when_record_existed(self, tmp_path: Path) -> None:
        v = _make_vendor()
        save_vendor(v, vendor_store_dir=tmp_path)
    ...
    def test_delete_returns_false_when_record_missing(self, tmp_path: Path) -> None:
        unknown = "00000000-0000-0000-0000-000000000000"
        assert delete_vendor(unknown, vendor_store_dir=tmp_path) is False

    def test_delete_rejects_id_shape_violation(self, tmp_path: Path) -> None:
        with pytest.raises(InvalidVendorIdError):
            delete_vendor("../etc/passwd", vendor_store_dir=tmp_path)

    def test_delete_only_removes_matching_record(self, tmp_path: Path) -> None:
...
class TestUuidCanonicalization:
    ...
    def test_save_rewrites_vendor_id_to_canonical(self, tmp_path: Path) -> None:
        v = _make_vendor()
