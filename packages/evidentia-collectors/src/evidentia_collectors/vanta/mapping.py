"""NIST 800-53 + OCC 2013-29 + FFIEC mappings for the Vanta collector.

Each mapping list represents the controls a particular Vanta-derived
finding satisfies. Mirrors the per-evidence-source mapping pattern
from `evidentia_collectors.{okta,databricks,snowflake}`.
...
banking-supervisor framing for third-party-risk programs that the
v0.7.9 TPRM module materializes. NIST 800-53 mappings are the
universal-substrate references that crosswalk-into-everything-else
per the v0.7.0 OLIR work.
"""
...
# Vanta vendor-inventory finding — captures that a vendor record
# exists in the operator's Vanta TPRM workspace; Evidentia surfaces
# it in the cross-framework evidence chain.
VENDOR_INVENTORY_MAPPINGS: list[ControlMapping] = [
    ...
    ControlMapping(
        ...
        justification=(
            "Vanta's vendor records typically carry the latest "
    ...
    ControlMapping(
        ...
        justification=(
            ...
            "categorization. The Vanta vendor inventory directly "
    ...
    ControlMapping(
        ...
        justification=(
            ...
            "as a core program element. Vanta-disclosed vendors "
            "feed the inventory."
        ),
    ),
    ControlMapping(
...
# Vanta — operator should review the underlying control gaps.
VENDOR_HIGH_RISK_MAPPINGS: list[ControlMapping] = [
    ControlMapping(
        ...
        relationship=OLIRRelationship.SUBSET_OF,
        justification=(
            "A high-risk vendor flag in Vanta represents a "
