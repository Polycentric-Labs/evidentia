...
| Release cadence | `collect release-cadence` | Public GitHub release facts, publication-time qualification, event identity and explicit local persistence | none |
...
Release cadence fixes the unauthenticated public GitHub Releases profile and returns a complete bounded `PollResult`. `collect_release_cadence(request)` is the Python entrypoint; `poll_release_bytes(request)` returns the accepted full JSON bytes. Polling does not open the evidence store unless `persist` is true. Core's `evaluate_release_series` reads saved publication families offline under an explicit window and spacing policy. Source observations preserve later fact changes without inflating publication counts. These results do not establish patch installation, remediation or compliance. See the [release-cadence guide](https://github.com/Polycentric-Labs/evidentia/blob/main/docs/wiki/2-guides/release-cadence.md) for authentication, source limits, storage outcomes and console downloads.
